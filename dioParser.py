...
class dioGrammar(Grammar):
    ...
    """
    special = Forward()
    source_hash__ = "1edfdb952fd0d7962be598907b1e8054"
    early_tree_reduction__ = CombinedParser.MERGE_LEAVES
    disposable__ = re.compile('(?:(?:(?:(?:(?:(?:(?:(?:(?:(?:(?:(?:(?:(?:sec_one$))|(?:sec_multi$))|(?:inscription$))|(?:inline$))|(?:tags$))|(?:letters_plain$))|(?:letters_extended$))|(?:combined$))|(?:precomposed$))|(?:separator$))|(?:phrase_terminator$))|(?:LF$))|(?:special$))|(?:EOF$)')
    ...
    letters = Alternative(letters_plain, letters_extended, cross, dots)
    omission = Series(Drop(Text("(")), OneOrMore(Alternative(letters, partial, space)), Drop(Text(")")), mandatory=1)
    phrases = Series(Alternative(letters, partial), phrase_terminator)
    inline = Alternative(phrases, partial, letters, unreadable, separator, space)
