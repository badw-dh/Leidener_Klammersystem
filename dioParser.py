...
class dioGrammar(Grammar):
    ...
    inline = Forward()
    special = Forward()
    source_hash__ = "d1cee186b83709999329d8f996632d29"
    early_tree_reduction__ = CombinedParser.MERGE_LEAVES
    disposable__ = re.compile('(?:(?:(?:(?:(?:(?:(?:(?:(?:(?:(?:(?:(?:(?:sec_one$))|(?:sec_multi$))|(?:sn$))|(?:inscription$))|(?:inline$))|(?:token$))|(?:tags$))|(?:letters_plain$))|(?:letters_extended$))|(?:combined$))|(?:precomposed$))|(?:special$))|(?:prettyspace$))|(?:EOF$)')
    ...
    supo = Text("°")
    breaktag = Text("<nl></nl>")
    letters_plain = OneOrMore(RegExp('[A-Za-z0-9](?!\\u0323|&#x323;)'))
    ...
    inscription = OneOrMore(Alternative(inline, special, prettyspace))
    ...
    snr = Series(Text("<snr>"), RegExp('[A-Z]+'), Text("."), prettyspace, Text("</snr>"))
    lno = Series(Text("<lno>"), inscription, Text("</lno>"), prettyspace, mandatory=1)
    entry = Series(Text("<entry>"), prettyspace, inscription, Text("</entry>"), mandatory=2)
    cell = Series(Text("<cell>"), prettyspace, OneOrMore(Series(entry, prettyspace)), Text("</cell>"))
    row = Series(Text("<row>"), prettyspace, OneOrMore(Series(cell, prettyspace)), Text("</row>"))
    table = Series(Text("<table>"), prettyspace, OneOrMore(Series(row, prettyspace)), Text("</table>"))
    lin = Series(Text("<lin>"), inscription, Text("</lin>"), prettyspace, mandatory=1)
    par = Series(Text("<par>"), prettyspace, OneOrMore(Alternative(lno, lin, table)), Text("</par>"), prettyspace)
    sec_one = Series(Text("<sec>"), prettyspace, par, Text("</sec>"), prettyspace)
    snt = Series(Text("<snt>"), prettyspace, OneOrMore(inline), prettyspace, Text("</snt>"))
    sn = OneOrMore(Series(Alternative(snt, snr), prettyspace))
    sec_multi = Series(Text("<sec>"), prettyspace, sn, prettyspace, par, Text("</sec>"), prettyspace)
